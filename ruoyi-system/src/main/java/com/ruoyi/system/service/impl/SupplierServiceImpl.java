...
public class SupplierServiceImpl implements ISupplierService {
    @Autowired
    private SupplierMapper postMapper;
    ...
    public boolean checkPostNameUnique(Supplier post) {
        ...
        Supplier info = postMapper.checkPostNameUnique(post.getPostName());
    ...
    public boolean checkPostCodeUnique(Supplier post) {
        Long postId = StringUtils.isNull(post.getPostId()) ? -1L : post.getPostId();
        Supplier info = postMapper.checkPostCodeUnique(post.getPostCode());
        if (StringUtils.isNotNull(info) && info.getPostId().longValue() != postId.longValue()) {
            return UserConstants.NOT_UNIQUE;
        }
        return UserConstants.UNIQUE;
    ...
    public int deletePostByIds(Long[] postIds) {
        for (Long postId : postIds) {
            Supplier post = selectPostById(postId);
